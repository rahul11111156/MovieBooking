...
public class MoviebookingappMicroserviceApplicationTests {
	...
	public void testAddMovie() throws AuthorizationException {
	 ...
	 public void testBookTicket() {
		  ...
		  ticket.setMovieName("Manifest");
		  ticket.setNumberOfTickets(1);
		  Set<String> tickets=new HashSet<String>();
		  tickets.add("A1");
		  ticket.setSeatNumbers(tickets);
		  ticket.setTheaterName("kumar");
		  ticket.setUserName("keerti");
		  Mockito.when(ticketService.bookMovie("Manifest", ticket)).thenReturn(new ResponseEntity<>(HttpStatus.CREATED));
		  assertEquals(new ResponseEntity<>(HttpStatus.CREATED),movieController.bookMovie("Manifest", ticket));
	 }
